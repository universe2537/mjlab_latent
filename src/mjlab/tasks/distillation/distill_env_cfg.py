...
def make_distillation_env_cfg() -> ManagerBasedRlEnvCfg:
    """Create the distillation environment configuration.

    This is a placeholder config that should be specialized with robot-specific
    parameters (e.g. motion files, anchor body, tracked bodies, contact geometry
    names, viewer body) to create the actual training environment.
    """
    # ── Observations ────────────────────────────────────────────────────────
    ...
    "joint_pos": JointPositionActionCfg(
        entity_name="robot",
        actuator_names=(".*",),
        scale=0.5,#G1_W_RACKET_ACTION_SCALE,
    ...
    # 论文 §3.2.2：online distillation 时对右手腕关节施加更大的常驻偏置，
    # 使 latent 空间对手腕扰动鲁棒，为 §3.3.2 hybrid wrist control 留出修正空间。
    "wrist_encoder_bias": EventTermCfg(
        mode="startup",
        func=dr.encoder_bias,
        params={
        "asset_cfg": SceneEntityCfg(
